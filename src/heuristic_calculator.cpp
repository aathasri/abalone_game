...
int HeuristicCalculator::W_COHESION_AI     = 50;
int HeuristicCalculator::W_COHESION_OPP    = 20;
int HeuristicCalculator::W_PUSH_AI         = 40; 
int HeuristicCalculator::W_PUSH_OPP        = 30;
int HeuristicCalculator::W_VULN_AI         = 20;
int HeuristicCalculator::W_VULN_OPP        = 60;
int HeuristicCalculator::W_ISOLATED_AI     = 40;
int HeuristicCalculator::W_ISOLATED_OPP    = 20;
int HeuristicCalculator::W_PROX_AI         = 50;
int HeuristicCalculator::W_PROX_OPP        = 0;
int HeuristicCalculator::W_MDIFF           = 850;
int HeuristicCalculator::W_EDGE_PUSH_AI = 100;
int HeuristicCalculator::W_EDGE_PUSH_OPP = 80;
...
int HeuristicCalculator::calculateHeuristic(const Board& b) const
{
    ...
    for (int i = 0; i < static_cast<int>(coords.size()); ++i) {
        ...
        if (occupant != AIuser && occupant != Opponent) continue;

        // For each direction, check if an opponent is one step from the edge
        for (int d = 0; d < static_cast<int>(MoveDirection::COUNT); ++d) {
            auto [dx, dy] = DirectionHelper::getDelta(d);
            int nx = x + dx;
            int ny = y + dy;
            int nnx = nx + dx;
            int nny = ny + dy;

            if (!b.validPosition(nx, ny)) continue;

            if (board[nx][ny] == (3 - occupant)) { // adjacent opponent
                if (!b.validPosition(nnx, nny)) {
                    edgePushThreat[occupant]++;
    ...
    int score = 0;
    score += (W_EDGE_PUSH_AI * edgePushThreat[AIuser]) - (W_EDGE_PUSH_OPP * edgePushThreat[Opponent]);
