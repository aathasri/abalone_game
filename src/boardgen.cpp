...
// helper function to extract the test number from the board file
std::string extractTestNumber(const std::string& filename) {
    size_t start = filename.find_first_of("0123456789");
    size_t end = filename.find_last_of("0123456789");
    return (start != std::string::npos && end != std::string::npos) ? filename.substr(start, end - start + 1) : "";
}
...
// helper function to convert the board state to a sorted string format
std::string boardToString(const std::unordered_map<std::string, char>& boardState) {
    std::vector<std::pair<std::string, char>> sortedBoard;
    for (const auto& [pos, color] : boardState) {
        sortedBoard.emplace_back(pos, color);
    }

    // Sort first by color (Black 'b' first, then White 'w'), then by row (A–I), then by column (ascending)
    std::sort(sortedBoard.begin(), sortedBoard.end(), [](const auto& a, const auto& b) {
        if (a.second != b.second) return a.second == 'b'; // Black first
        if (a.first[0] != b.first[0]) return a.first[0] < b.first[0]; // Row order A–I
        return std::stoi(a.first.substr(1)) < std::stoi(b.first.substr(1)); // Column order ascending
    });

    std::stringstream ss;
    for (const auto& [pos, color] : sortedBoard) {
        ss << pos << color << ",";
    }
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <board_file> <moves_file>" << std::endl;
        return 1;
    }

    std::string boardFileName = argv[1];
    std::string movesFileName = argv[2];
    std::string testNumber = extractTestNumber(boardFileName);
    std::string outputFileName = "Test" + testNumber + ".board";

    simulateMoves(boardFileName, movesFileName, outputFileName);
