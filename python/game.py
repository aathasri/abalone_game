...
class SelectableTile(QGraphicsEllipseItem):
    ...
    def _send_to_backend(self, start_coords, direction):
        ...
        human_color = "black" if self.settings["color"] == "Black" else "white"
        ai_color = "white" if human_color == "black" else "black"
        time_limit = self.time_limit_p1 if human_color == "black" else self.time_limit_p2
        ...
        if self.current_player != human_color:
            QMessageBox.warning(None, "Wrong Turn", f"It's {self.current_player}'s turn!")
            return
        ...
        try:
            ...
            player_timers[human_color] += duration
            player_moves[human_color] += 1
            move_str = f"{self._format_coords(start_coords)} > {direction}"
            log_entry = f"[MOVE] {human_color.upper()}: {move_str} | Time: {duration:.2f}s"
            game_log.append(log_entry)
            if SelectableTile.log_callback:
                SelectableTile.log_callback(log_entry)

            log_entry = f"[MOVE] {ai_color.upper()} (AI): Processed | Time: {ai_time:.2f}s"
            game_log.append(log_entry)
            if SelectableTile.log_callback:
                SelectableTile.log_callback(log_entry)
            player_timers[ai_color] += ai_time
            player_moves[ai_color] += 1

            self.current_player = "black" if human_color == "white" else "white"  # Black goes next
            current_turn_start = time.time()
    ...
    def _undo_last_move(self):
        ...
        try:
            ...
            if player_moves[ai_color] > 0:
                player_timers[ai_color] -= float(game_log[-1].split("Time: ")[1].split("s")[0])
                player_moves[ai_color] -= 1
                game_log.pop()
            if player_moves[human_color] > 0:
                player_timers[human_color] -= float(game_log[-1].split("Time: ")[1].split("s")[0])
                player_moves[human_color] -= 1
                game_log.pop()

            black_pos, white_pos = SelectableTile.backend_game.get_current_state()
            SelectableTile._update_board(black_pos, white_pos)
            self.current_player = "black"  # Black always starts after undo
            current_turn_start = time.time()
    ...
    def _end_game(self, black_pos, white_pos, reason=None):
        ...
        winner = "White" if len(black_pos) <= 8 else "Black" if len(white_pos) <= 8 else "None"
...
class AbaloneBoardView(QGraphicsView):
    def __init__(self, backend_game, settings):
        ...
        SelectableTile.settings = settings
        human_color = "black" if settings["color"] == "Black" else "white"
        ai_color = "white" if human_color == "black" else "black"
        SelectableTile.current_player = "black"  # Black always starts
        ...
        # If AI is Black, make a random first move
        if ai_color == "black":
            status, black_pos, white_pos, ai_time = backend_game.make_player_and_ai_move([], "RANDOM")
            if status == "VALID":
                SelectableTile._update_board(black_pos, white_pos)
                player_timers["black"] += ai_time
                player_moves["black"] += 1
                game_log.append(f"[MOVE] BLACK (AI): Random Initial | Time: {ai_time:.2f}s")
                if SelectableTile.log_callback:
                    SelectableTile.log_callback(game_log[-1])
                SelectableTile.current_player = "white"  # Human (White) moves next
            else:
                print(f"[DEBUG] Random AI move failed: status={status}")
            current_turn_start = time.time()
            if SelectableTile.update_callback:
                SelectableTile.update_callback()
    ...
    def _update_labels(self):
        ...
        human_color = "black" if SelectableTile.settings["color"] == "Black" else "white"
        ai_color = "white" if human_color == "black" else "black"
        self.p1_box.setText(
            f"Moves: {player_moves[human_color]}\nPoints: {player_points[human_color]}\n{'⚫' if human_color == 'black' else '⚪'} PLAYER 1\n{'YOUR TURN' if SelectableTile.current_player == human_color else ''}\n{player_timers[human_color]:.2f}"
        )
        self.p2_box.setText(
            f"Moves: {player_moves[ai_color]}\nPoints: {player_points[ai_color]}\n{'⚪' if ai_color == 'white' else '⚫'} COMP\n{'YOUR TURN' if SelectableTile.current_player == ai_color else ''}\n{player_timers[ai_color]:.2f}"
        )
